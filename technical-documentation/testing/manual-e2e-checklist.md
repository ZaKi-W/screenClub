...
2. The app is single-instance per `userData` path. If a leftover Electron/ScreenClub process still holds the lock, stop that process before relaunching; a second launch can exit successfully without opening a window. The lock is held by the OS and is released when the process dies, so there is nothing to delete on disk.
...
4. Grant computer-use access to the process name that actually owns the window: `electron.exe` or `Electron.app` for a dev build, and `ScreenClub.exe` or `ScreenClub.app` for a packaged build. Do not grant access only to the installed app name when testing a dev build.

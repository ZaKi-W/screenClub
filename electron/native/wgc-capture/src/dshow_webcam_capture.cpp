...
bool DirectShowWebcamCapture::initialize(
    ...
    if (!succeeded(impl_->graph->AddFilter(impl_->captureFilter.Get(), L"ScreenClub Webcam Source"),
    ...
    if (!succeeded(impl_->graph->AddFilter(impl_->sampleGrabberFilter.Get(), L"ScreenClub Webcam Sample Grabber"),
    ...
    if (!succeeded(impl_->graph->AddFilter(impl_->nullRenderer.Get(), L"ScreenClub Webcam Null Renderer"),
